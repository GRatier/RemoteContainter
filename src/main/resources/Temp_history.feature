Feature: View temperature history of container
  ...
  @tag1
  Scenario: Find temperature history based on container number 
    Given a client
    And a container with container number 123
    And a has travelled status true
    And journey history is visible
    When client clicks on temperature history of container number 123
    Then all past temperature levels of container are displayed
